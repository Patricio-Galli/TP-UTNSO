...
int main() {
	...
	while(!salir) {
		...
		variable++;
		log_info(logger,"Entro al switch");
		/*
		funcion_consola = mapStringToEnum(primer_palabra(buffer_consola));*/
		switch (variable) {
		case 1:
			log_info(logger,"CASE 1 INICIAR_PATOTA");
			funcion_consola = INICIAR_PATOTA;
			break;
		case 2:
			log_info(logger,"CASE 2 INICIAR_PATOTA");
			funcion_consola = INICIAR_PATOTA;
			break;
		case 3:
			funcion_consola = INICIAR_PLANIFICACION;
			break;
		case 4:
			funcion_consola = INICIAR_PATOTA;
			break;
		...
		switch(funcion_consola) {
			case INICIAR_PATOTA:
				log_info(logger,"Iniciar patota");
				...
				if(RAM_ACTIVADA || MONGO_ACTIVADO) {
					...
					if(RAM_ACTIVADA) {
						log_info(logger,"Enviar mensaje");
			...
			case EXPULSAR_TRIPULANTE:
				...
				expulsar_tripulante(id_trip, id_patota);
				break;
...
parametros_iniciar_patota* obtener_parametros(char** input) {//todo realizar validaciones para lectura de archivos y parametros validos
	...
	log_info(logger,"Paso 0");
	bool valida = true;
	log_info(logger,"Paso 0.5");
