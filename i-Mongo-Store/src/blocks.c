	...
	char DIR_metadata[90];
		if(caracter_llenado=='O'){
			...
			FILE* metadata;
				...
				if(metadata!=NULL){
					...
					int temp;
					char* temp1;
					for(int i=1;i<cantidad_original_bloques+1;i++){
						temp1=buffer[i];
						temp=temp1[0]-48;
						cantidad_caracteres=borrar_caracter_en_bloque(caracter_llenado,cantidad_caracteres,buffer[i],cantidad_caracteres);
						pthread_mutex_lock(&actualizar_bitmap);
		...
		int bloques_reducidos;
			...
			if(bloques_sobrantes>0){//si hay mas bloques de los que necesito, debo sacar bloques}
							...
							for(int i=0;i<string_length(bloques_originales);i++){
								printf("%c",bloques_originales[i]);
							}printf("\n");
							bloques_originales[0]=','; //porque si no copia el [
							...
							char** buffer=string_split(bloques_originales,",");
								...
								for(int i=1;i<bloques_reducidos+1;i++){
									string_append(&bloques_a_dejar,buffer[i]);
									if(i+1==bloques_reducidos+1){;
										string_append(&bloques_a_dejar,"]");
									}
									else
										string_append(&bloques_a_dejar,",");
								...
								for(int i=bloques_reducidos+1;i<cantidad_original_bloques+1;i++){
											temp1=buffer[i];
											temp=temp1[0]-48;
											int aux=cantidad_caracteres;
											cantidad_caracteres=borrar_caracter_en_bloque(caracter_llenado,cantidad_caracteres,buffer[i],size_original);
											size_original-=aux-cantidad_caracteres;
											pthread_mutex_lock(&actualizar_bitmap);
											bitarray_clean_bit(bitmap,temp);
											pthread_mutex_unlock(&actualizar_bitmap);
