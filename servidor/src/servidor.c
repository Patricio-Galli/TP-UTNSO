...
int main(void) {
	...
	agregar_parametro_a_mensaje(mensaje_out, (void *)4040, ENTERO);
	...
	log_info(logger, "Recibi cant_tareas %d", (int)list_get(mensaje_in, 3));
	log_info(logger, "Tarea 1: %s", (char *)list_get(mensaje_in, 4));
	log_info(logger, "Tarea 2: %s", (char *)list_get(mensaje_in, 5));
